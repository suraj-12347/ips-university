import React, {
  useEffect,
  useState,
} from "react";

import { X } from "lucide-react";
import DotedRing from "./DotedRing";

import { useLanguage } from "../context/LanguageContext";

const EnquiryPopup = () => {
  const [open, setOpen] =
    useState(false);

  const { t } = useLanguage();

  useEffect(() => {
    const firstTimer = setTimeout(() => {
      setOpen(true);
    }, 5000);

    const interval = setInterval(() => {
      setOpen(true);
    }, 2000000000);

    return () => {
      clearTimeout(firstTimer);
      clearInterval(interval);
    };
  }, []);

  return (
    <>
      {open && (
        <div
          className="
            fixed
            inset-0
            z-[999]
            flex
            items-center
            justify-center
            bg-black/60
            backdrop-blur-sm
            px-4
            top-30
          "
        >

          {/* POPUP */}
          <div
            className="
              relative
              w-full
              max-w-[1000px]
              h-[400px]
              overflow-hidden
              shadow-2xl
              animate-[popup_.4s_ease]
              flex
              flex-col
              lg:flex-row
            "
            style={{
              background:
                "linear-gradient(135deg,var(--forest),var(--grass))",
            }}
          >

            {/* DOTS */}
            <div className="absolute -left-35 -top-35 rotate-180">
              <DotedRing
                height={450}
                width={450}
              />
            </div>

            <div className="absolute right-10 -bottom-80 ">
              <DotedRing
                height={700}
                width={700}
              />
            </div>

            {/* CLOSE BTN */}
            <button
              onClick={() =>
                setOpen(false)
              }
              className="
                absolute
                top-4
                right-4
                z-50
                w-10
                h-10
                rounded-full
                bg-white/10
                hover:bg-white/20
                flex
                items-center
                justify-center
                text-white
                transition-all
                cursor-pointer
              "
            >
              <X size={20} />
            </button>

            {/* LEFT CONTENT */}
            <div
              className="
                hidden
                lg:flex
                w-[42%]
                flex-col
                justify-center
                px-10
                relative
                z-20
              "
            >

              <p className="text-white/70 uppercase tracking-[4px] text-xs mb-3">
                IPS University
              </p>

              <h2 className="text-4xl font-extrabold text-white leading-tight">
                {t.admission}
                <br />
                {t.enquiry}
              </h2>

              <p className="text-white/80 mt-5 leading-7 text-sm">
                {t.enquiryDesc}
              </p>

              <div className="mt-6 flex flex-col gap-3 text-sm">

                <div className="flex items-center gap-3 text-white">
                  <span className="w-2 h-2 rounded-full bg-white"></span>
                  <p>
                    {t.placementAssist}
                  </p>
                </div>

                <div className="flex items-center gap-3 text-white">
                  <span className="w-2 h-2 rounded-full bg-white"></span>
                  <p>
                    {t.aicteCourses||"AICTE Aproved Course"}
                  </p>
                </div>

                <div className="flex items-center gap-3 text-white">
                  <span className="w-2 h-2 rounded-full bg-white"></span>
                  <p>
                    {t.smartCampus||"80 Acres Smart Campus"}
                  </p>
                </div>

              </div>
            </div>

            {/* RIGHT FORM */}
            <div
              className="
                w-full
                lg:w-[58%]
                flex
                items-center
                justify-center
                p-4
                md:p-6
                relative
                z-20
              "
            >

              <div
                className="
                  w-full
                  bg-transparent
                  py-10
                  rounded-[24px]
                  p-5
                "
              >

                <h3 className="text-2xl font-bold text-[var(--white)] ">
                  {t.getStarted}
                </h3>

                <p className="text-white/90 text-sm mb-2">
                  {t.fillDetails}
                </p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">

                  <input
                    type="text"
                    placeholder={
                      t.fullName
                    }
                    className="
                      w-full
                      h-[48px]
                      rounded-xl
                      border
                      border-gray-200
                      px-4
                      outline-none
                      focus:border-[var(--forest)]
                      bg-white/80
                      text-[var(--forest)]
                    "
                  />

                  <input
                    type="tel"
                    placeholder={
                      t.mobileNumber||"Phone"
                    }
                    className="
                      w-full
                      h-[48px]
                      rounded-xl
                      border
                      border-gray-200
                      bg-white/80
                      text-[var(--forest)]
                      px-4
                      outline-none
                      focus:border-[var(--forest)]
                    "
                  />

                  <input
                    type="email"
                    placeholder={
                      t.emailAddress||"Enter Your email"
                    }
                    className="
                      w-full
                      h-[48px]
                      rounded-xl
                      border
                      border-gray-200
                      px-4
                      outline-none
                      focus:border-[var(--forest)]
                      bg-white/80
                      text-[var(--forest)]
                    "
                  />

                  <select
                    className="
                      w-full
                      h-[48px]
                      rounded-xl
                      border
                      border-gray-200
                      px-4
                      outline-none
                      focus:border-[var(--forest)]
                      bg-white/80
                      text-[var(--forest)]
                      text-gray-600
                    "
                  >
                    <option>
                      {t.selectCourse}
                    </option>

                    <option>B.Tech</option>
                    <option>BCA</option>
                    <option>BBA</option>
                    <option>MBA</option>
                    <option>B.Pharm</option>
                    <option>MCA</option>
                  </select>

                  <textarea
                    rows="3"
                    placeholder={
                      t.yourMessage||"type your Message"
                    }
                    className="
                      md:col-span-2
                      w-full
                      rounded-xl
                      border
                      border-gray-200
                      p-4
                      outline-none
                      resize-none
                      focus:border-[var(--forest)]
                      bg-white/80
                      text-[var(--forest)]
                    "
                  ></textarea>

                  <button
                    className="
                      md:col-span-2
                      w-full
                      h-[50px]
                      rounded-xl
                      text-[var(--forest)]
                      font-semibold
                      bg-[var(--white)]
                      hover:scale-[1.02]
                      transition-all
                      cursor-pointer
                    "
                  >
                    Submit
                  </button>

                </div>

              </div>
            </div>
          </div>
        </div>
      )}

      {/* ANIMATION */}
      <style>
        {`
          @keyframes popup{
            from{
              opacity:0;
              transform:scale(.9) translateY(20px);
            }

            to{
              opacity:1;
              transform:scale(1) translateY(0);
            }
          }
        `}
      </style>
    </>
  );
};

export default EnquiryPopup;