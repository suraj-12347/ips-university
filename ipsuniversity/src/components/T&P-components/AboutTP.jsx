import React from "react";
import {
  ArrowRight,
  Award,
  FileCheck2,
  Trophy,
  Briefcase,
  ShieldCheck,
  Users,
  GraduationCap,
  Building2,
  MessageSquare,
  Target,
  Handshake,
  BookOpen,
  Presentation,
  Globe2,
  ClipboardList,
} from "lucide-react";
import ctm1900 from '../../images1/adminbuild.jpeg'
import tp from '../../images1/tpcell.png'
import { Link } from "react-router-dom";

/* ==========================================
   DATA
========================================== */

// Quick-glance headline figures shown as stat cards
const quickStats = [
  { icon: Briefcase, value: "150+", label: "Recruiting Companies" },
  { icon: Award, value: "12 LPA", label: "Highest Package Offered" },
  { icon: Users, value: "1000+", label: "Students Placed Yearly" },
  { icon: Target, value: "92%", label: "Placement Assistance Rate" },
];

// Broad categories of T&P activities offered
const categories = [
  {
    title: "Pre-Placement Training",
    text: "Aptitude, technical and soft-skills training conducted from the first year onwards to build a strong foundation for campus placements.",
  },
  {
    title: "Internships & Industrial Training",
    text: "Structured summer and winter internships with reputed organisations, giving students real workplace exposure before they graduate.",
  },
  {
    title: "Campus Recruitment Drives",
    text: "On-campus and off-campus hiring drives with leading companies across engineering, management, commerce and allied disciplines.",
  },
  {
    title: "Career Counselling & Higher Studies",
    text: "One-on-one mentoring for career planning, along with guidance for students aiming for higher studies in India and abroad.",
  },
];

// Full list of T&P Cell activities/services
const placementServices = [
  {
    icon: MessageSquare,
    badge: "Year Round",
    title: "Soft Skills & Communication",
    desc: "Workshops on communication, group discussions and personality development to make students interview-ready.",
    category: "Training",
  },
  {
    icon: ClipboardList,
    badge: "Ongoing",
    title: "Aptitude & Technical Training",
    desc: "Regular practice sessions and mock tests covering quantitative aptitude, reasoning and core technical subjects.",
    category: "Training",
  },
  {
    icon: Presentation,
    badge: "Mock Sessions",
    title: "Mock Interviews & GD",
    desc: "Simulated interviews and group discussions conducted with faculty and industry experts before actual recruitment drives.",
    category: "Training",
  },
  {
    icon: Building2,
    badge: "Every Semester",
    title: "Internship Placement",
    desc: "Support in securing summer and winter internships with corporates, startups and public sector organisations.",
    category: "Internship",
  },
  {
    icon: Handshake,
    badge: "150+ Partners",
    title: "Industry Tie-Ups",
    desc: "MoUs and long-term partnerships with companies for internships, live projects, guest lectures and placements.",
    category: "Placement",
  },
  {
    icon: Briefcase,
    badge: "On/Off Campus",
    title: "Campus Recruitment",
    desc: "End-to-end coordination of recruitment drives, from company registration to final offer letters for students.",
    category: "Placement",
  },
  {
    icon: BookOpen,
    badge: "Resume Support",
    title: "Resume & Profile Building",
    desc: "One-on-one sessions to help students build strong resumes and professional profiles tailored to their target roles.",
    category: "Training",
  },
  {
    icon: Globe2,
    badge: "Guidance",
    title: "Higher Studies Counselling",
    desc: "Guidance and test-prep support for students planning to pursue higher education in India or abroad.",
    category: "Counselling",
  },
];

const categoryStyles = {
  Training: "bg-[#ecf39e] text-[#31572c]",
  Placement: "bg-[#31572c]/10 text-[#31572c]",
  Internship: "bg-gray-100 text-gray-600",
  Counselling: "bg-[#d4a14d]/15 text-[#8a6112]",
};

// General terms every student should know about the T&P Cell
const generalTerms = [
  {
    icon: ClipboardList,
    title: "Mandatory Registration",
    text: "All eligible final-year students must register with the T&P Cell to be considered for any campus placement drive.",
  },
  {
    icon: ShieldCheck,
    title: "Attendance & Eligibility",
    text: "Students must meet the minimum attendance and academic criteria set by recruiters to sit for their respective drives.",
  },
  {
    icon: Award,
    title: "One Offer Policy",
    text: "Students who accept an offer through the T&P Cell are generally not permitted to sit for further core drives in the same cycle.",
  },
  {
    icon: FileCheck2,
    title: "Code of Conduct",
    text: "Professional conduct, punctuality and honesty during all recruitment processes are mandatory for every registered student.",
  },
];

const AboutTP = () => {
  return (
    <div className="w-full bg-white mt-30">

      {/* ========================================================= */}
      {/* HERO SECTION */}
      {/* ========================================================= */}

      <section className="relative h-[420px] flex items-center overflow-hidden" style={{  backgroundImage:  `url(${tp})`, backgroundSize: "cover",  backgroundPosition: "center",  }}  >
        <div className=" absolute inset-0 bg-gradient-to-r from-[#31572c]/90 to-transparent" />

        <div className="relative max-w-7xl  px-6 lg:px-10 text-white">

          <div className="text-sm uppercase tracking-[3px] text-[#cde8a3] mb-5">
            Careers & Industry Relations
          </div>

          <h1 className="text-5xl md:text-6xl font-bold leading-tight">
            Training & Placement Cell
          </h1>

          <p className="mt-6 text-lg text-gray-200 max-w-3xl leading-8">
            IPS University's Training & Placement Cell bridges the gap between
            academics and industry, preparing students with the right skills,
            exposure and opportunities to launch successful careers.
          </p>

          <div className="flex items-center gap-2 mt-8 text-sm">

  <Link
    to="/"
    className="hover:text-[var(--grass)] transition"
  >
    Home
  </Link>


  <ArrowRight size={16} />


  <Link
    to="/admission"
    className="hover:text-[var(--grass)] transition"
  >
    Admission
  </Link>


  <ArrowRight size={16} />


  <span className="text-[#cde8a3] font-semibold">
    Training & Placement Cell
  </span>

</div>
        </div>
      </section>

      {/* ========================================================= */}
      {/* INTRO + FLAGSHIP HIGHLIGHT */}
      {/* ========================================================= */}

   <section className="py-20">
  <div className="max-w-7xl mx-auto px-6">

    <div className="relative h-[520px] rounded-[30px] overflow-hidden shadow-2xl">

      {/* Background Image */}
      <img
  src="https://images.unsplash.com/photo-1521737604893-d14cc237f11d?auto=format&fit=crop&w=1600&q=80"
  alt="Placement Drive"
  className="absolute inset-0 w-full h-full object-cover"
/>

      {/* Dark Overlay */}
      <div className="absolute inset-0 bg-black/20"></div>


      {/* White Gradient Inset */}
      <div
        className="
          absolute 
          left-0 
          top-0 
          h-full 
          w-[58%]
          bg-gradient-to-r 
          from-white 
          via-white/95 
          to-white/0
        "
      ></div>


      {/* Content */}
      <div className="relative z-10 h-full flex items-center">

        <div className="max-w-lg pl-12 pr-10">

          <span className="flex items-center gap-2 text-[#578B07] font-semibold tracking-[3px] uppercase text-xs">

            <div className="w-2 h-2 rounded-full bg-[#8BAE45]" />

            Our Commitment

          </span>


          <h2 className="
            text-4xl
            leading-[44px]
            font-bold
            text-[#31572c]
            mt-5
          ">

            Career Readiness

            <span className="block text-[#7ea33d] italic font-medium">
              From Classroom To Corporate
            </span>

          </h2>


          <div className="w-20 h-1 bg-[#8BAE45] rounded-full mt-6"></div>


          <p className="mt-6 text-gray-700 text-sm leading-7 max-w-md">

            The Training & Placement Cell works closely with every department
            to identify student strengths early and build a structured path
            of training, internships and placement support across all four
            years of study.

          </p>


          <p className="mt-4 text-gray-700 text-sm leading-7 max-w-md">

            Every registered student receives personalised mentoring, mock
            interview practice and access to our growing network of hiring
            partners across India.

          </p>


          {/* Info Block */}
          <div className="flex items-center gap-4 mt-8">

            <div className="
              w-12 
              h-12 
              rounded-full 
              bg-[#31572c] 
              text-white 
              flex 
              items-center 
              justify-center
            ">
              <Trophy size={22} />
            </div>


            <div>

              <h3 className="font-bold text-lg text-[#31572c]">
                150+ Companies On Campus
              </h3>

              <p className="text-sm text-[#578B07]">
                Every Academic Year Across Streams
              </p>

            </div>

          </div>


        </div>

      </div>


    </div>

  </div>
</section>

      {/* ========================================================= */}
      {/* QUICK STATS */}
      {/* ========================================================= */}

      <section className="pb-24">

        <div className="max-w-7xl mx-auto px-6 lg:px-10">

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">

            {quickStats.map((item, index) => {
              const Icon = item.icon;
              return (
                <div
                  key={index}
                  className="bg-white rounded-3xl border border-gray-200 p-10 hover:shadow-xl transition duration-300"
                >

                  <div className="w-16 h-16 rounded-2xl bg-[#ecf39e] flex items-center justify-center">

                    <Icon className="text-[#31572c]" size={30} />

                  </div>

                  <h2 className="text-4xl font-bold mt-8 text-[#31572c]">
                    {item.value}
                  </h2>

                  <p className="mt-3 text-gray-600">
                    {item.label}
                  </p>

                </div>
              );
            })}

          </div>

        </div>

      </section>

      {/* ========================================================= */}
      {/* CATEGORIES AT A GLANCE */}
      {/* ========================================================= */}
<section className="py-24 bg-[#f8faf7]">
  <div className="max-w-7xl mx-auto px-6 lg:px-10">

    <div className="text-center mb-14">
      <span className="uppercase tracking-[4px] text-[#8BAE45] font-semibold">
        What We Do
      </span>

      <h2 className="text-4xl font-bold text-[#31572c] mt-3">
        A Complete Career Development Ecosystem
      </h2>

      <p className="max-w-3xl mx-auto mt-5 text-gray-600 leading-8">
        From the first year to final placement, the T&P Cell walks alongside
        every student — building skills, providing exposure, and opening
        doors to the right opportunities.
      </p>
    </div>

    <div className="grid md:grid-cols-2 gap-8">

      {categories.map((item, index) => (
        <div
          key={index}
          className="bg-white rounded-3xl p-8 shadow-md border border-gray-100"
        >
          <h3 className="text-2xl font-semibold text-[#31572c]">
            {item.title}
          </h3>

          <p className="text-gray-600 mt-4 leading-8">
            {item.text}
          </p>
        </div>
      ))}

    </div>

  </div>
</section>

{/* ========================================================= */}
{/* FULL LIST OF SERVICES */}
{/* ========================================================= */}

<section className="py-24 bg-white">
  <div className="max-w-7xl mx-auto px-6">

    {/* Heading */}
    <div className="text-center mb-14">
      <span className="uppercase tracking-[4px] text-sm font-semibold text-[#31572c]">
        Our Services
      </span>

      <h2 className="text-4xl lg:text-5xl font-bold text-[#1d3d20] mt-4">
        How The T&P Cell Supports You
      </h2>

      <div className="w-24 h-1 bg-[#d4a14d] mx-auto mt-5 rounded-full"></div>
    </div>

    {/* Cards */}
    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">

      {placementServices.map((item, index) => {
        const Icon = item.icon;
        return (
          <div
            key={index}
            className="
              group
              bg-white
              rounded-[26px]
              overflow-hidden
              shadow-lg
              border
              border-gray-100
              hover:-translate-y-2
              hover:shadow-2xl
              transition-all
              duration-500
              p-7
            "
          >

            <div className="flex items-center justify-between">
              <div className="w-12 h-12 rounded-2xl bg-[#31572c] text-[#ecf39e] flex items-center justify-center">
                <Icon size={22} />
              </div>

              <span
                className={`text-xs font-semibold px-3 py-1 rounded-full ${categoryStyles[item.category]}`}
              >
                {item.category}
              </span>
            </div>

            <p className="mt-6 text-sm font-semibold text-[#578B07]">
              {item.badge}
            </p>

            <h3 className="text-lg font-bold text-[#1d3d20] mt-1">
              {item.title}
            </h3>

            <p className="text-gray-600 text-sm mt-3 leading-6">
              {item.desc}
            </p>

          </div>
        );
      })}

    </div>
  </div>
</section>

{/* ========================================================= */}
{/* CALL TO ACTION */}
{/* ========================================================= */}

<section className="pb-24">
  <div className="max-w-7xl mx-auto px-6 lg:px-10">

    <div className="relative rounded-[36px] overflow-hidden shadow-2xl min-h-[320px]">

      {/* Right Background Image */}
      <img
        src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=1600&q=80"
        alt="Students in Interview"
        className="
          absolute
          inset-0
          w-full
          h-full
          object-cover
        "
      />


      {/* Left Green Blend */}
      <div
        className="
          absolute
          inset-0
          bg-gradient-to-r
          from-[#31572c]
          via-[#31572c]/90
          via-60%
          to-[#31572c]/20
        "
      ></div>


      {/* Soft Glow */}
      <div className="absolute -top-20 -right-20 w-72 h-72 bg-[#8BAE45]/30 rounded-full blur-3xl"></div>
      <div className="absolute -bottom-24 -left-24 w-80 h-80 bg-white/10 rounded-full blur-3xl"></div>


      {/* Content */}
      <div
        className="
          relative
          z-10
          px-8
          md:px-16
          py-14
          flex
          flex-col
          lg:flex-row
          items-center
          justify-between
          gap-10
        "
      >

        <div className="max-w-3xl">

          <span className="uppercase tracking-[3px] text-[#cfe8a9] font-semibold text-sm">
            For Recruiters & Students
          </span>


          <h2 className="text-4xl font-bold text-white mt-4 leading-tight">
            Let's Build Your
            <br />
            Career Story Together.
          </h2>


          <p className="text-[#d8e8d2] mt-5 max-w-2xl leading-7 text-sm md:text-base">
            Whether you're a student preparing for placements or a company
            looking to hire from IPS University, our T&P Cell is here to
            connect you with the right opportunity.
          </p>

        </div>


        <button
          className="
            bg-white
            text-[#31572c]
            px-8
            py-4
            rounded-full
            font-semibold
            hover:bg-[#ecf39e]
            transition
            duration-300
            shadow-lg
            whitespace-nowrap
          "
        >
          Contact T&P Cell →
        </button>


      </div>

    </div>

  </div>
</section>

{/* ========================================================= */}
{/* GENERAL TERMS */}
{/* ========================================================= */}



<section className="py-24 bg-[#f8faf7]">
  <div className="max-w-7xl mx-auto px-6 lg:px-10">

    <div className="text-center mb-16">

      <span className="uppercase tracking-[4px] text-[#8BAE45] font-semibold">
        Before You Register
      </span>

      <h2 className="text-4xl font-bold text-[#31572c] mt-4">
        Placement Policies You Should Know
      </h2>

      <p className="max-w-3xl mx-auto mt-5 text-gray-600 leading-8">
        Every student registered with the Training & Placement Cell is
        expected to follow a common set of guidelines that keep the process
        fair and transparent for all recruiters and students alike.
      </p>

    </div>

    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">

      {generalTerms.map((item, index) => {
        const Icon = item.icon;
        return (
          <div
            key={index}
            className="bg-white rounded-3xl p-8 shadow-md hover:shadow-xl transition"
          >
            <Icon className="text-[#31572c] w-10 h-10 mb-5" />
            <h3 className="text-xl font-bold text-[#31572c]">
              {item.title}
            </h3>
            <p className="text-gray-600 mt-3 leading-7">
              {item.text}
            </p>
          </div>
        );
      })}

    </div>

  </div>
</section>
      {/*
          NEXT SECTION:

          1. Our Recruiters (Logo Grid / Marquee)
          2. Placement Statistics Year-Wise (Charts)
          3. Student Testimonials
          4. T&P Cell Team & Contact / Grievance Redressal
      */}

    </div>
  );
};

export default AboutTP;